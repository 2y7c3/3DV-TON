...
class DensePose:
    ...
    def __init__(self, model_path="./checkpoints/densepose_", device="cuda"):
        ...
        self.config_path = os.path.join(model_path, 'densepose_rcnn_R_101_FPN_DL_s1x.yaml')
        self.model_path = os.path.join(model_path, 'model_final_844d15.pkl')
    ...
    def create_context(self, cfg, output_path=None):
    ...
    def __call__(self, image_or_path, resize=512) -> Image.Image:
        ...
        # tmp_path = f"./densepose_lower/tmp/"
        ...
        assert isinstance(image_or_path, Image.Image)
        
        image_or_path = np.array(image_or_path)
        img = cv2.cvtColor(image_or_path, cv2.COLOR_RGB2BGR)
        
        # file_list = self._get_input_file_list(image_path)
        # assert len(file_list), "No input images found!"
        context = self.create_context(self.cfg, output_path=None)
        ...
        with torch.no_grad():
            ...
            try:
                dense_gray = self.execute_on_outputs(context, {"file_name": None, "image": img}, outputs)
            except Exception as e:
                dense_gray = Image.new('L', (1, 1))
                # print(e)
