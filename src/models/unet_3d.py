...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        points_fea: Optional[torch.Tensor] = None,
        ...
        if any(s % default_overall_up_factor != 0 for s in sample.shape[-2:]):
            print(default_overall_up_factor, sample.shape[-2:])
    ...
    def from_pretrained_2d(
        ...
        #state_dict=torch.load("/mnt/disk1/fzx/Models/vivid/ablation1/denoising_unet-7524.pth", map_location="cpu",weights_only=True)
   
    
        # load the motion module weights
        if motion_module_path is not None and motion_module_path.exists() and motion_module_path.is_file():
        ...
        # state_dict["conv_in.weight"].shape = [320,4,3,3]
        # state_dict["conv_in.bias"].shape = [320]
        print(unet_additional_kwargs)
        if state_dict["conv_in.weight"].shape[1] != unet_additional_kwargs["in_channels"]:
            print("new conv_in")
            state_dict["conv_in.weight"] = torch.zeros((320,unet_additional_kwargs["in_channels"],3,3), dtype=torch.float32)
            state_dict["conv_in.bias"] = torch.zeros((320), dtype=torch.float32)

        # cnt = 0
        # nums = 0
        # for k,v in state_dict.items():
        #     if ("attn2" in k) or ("0.norm2" in k):
        #         print(k, state_dict[k].shape)
        #         cnt+=1
        #         nums+=state_dict[k].numel()
        #         # _k = k.replace("attn2", "attn1")
        #         # print(k, _k)
        #         # state_dict[k] = state_dict[_k].clone()
        # print(cnt, nums / 1e6)
